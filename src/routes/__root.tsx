...
import { MinttenLeaf, MinttenWordmark } from "@/components/BrandLogo";
...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { title: `${APP_FULL_NAME} — Billing & Inventory ERP` },
      {
        name: "description",
        content: "Keyboard-first desktop billing, inventory, and accounting software.",
      },
      { property: "og:title", content: `${APP_FULL_NAME} — Billing & Inventory ERP` },
      {
      ...
      // Keep in step with --gradient-brand's first stop in src/styles.css and
      // theme_color in public/manifest.webmanifest.
      { name: "theme-color", content: "#112621" },
    ],
    links: [
      ...
      // regular favicon above) — it wants a real PNG at 180x180, which is
      // what this now is. Regenerate it (and favicon.ico / icon-*.png, all
      // drawn from the same mark) if the logo ever changes.
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-background gap-4">
      <div className="h-14 w-14 rounded-2xl bg-gradient-brand text-brand-foreground flex items-center justify-center shadow-elevated">
        <MinttenLeaf className="h-7 w-7" />
      </div>
      <div className="text-center flex flex-col items-center">
        <MinttenWordmark size="md" tagline />
